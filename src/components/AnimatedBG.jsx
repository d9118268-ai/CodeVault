    ...
    <>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { background: #000; font-family: 'Segoe UI', sans-serif; overflow-x: hidden; min-height: 100vh; }
        ...
        .lang-logo { display: flex; flex-direction: column; align-items: center; gap: 6px; opacity: 0.18; filter: drop-shadow(0 0 8px currentColor); }
        .lang-logo svg { width: 48px; height: 48px; }
        .lang-logo span { font-size: 11px; font-weight: 700; letter-spacing: 1px; opacity: 0.8; }
        .page-content { position: relative; z-index: 10; min-height: 100vh; display: flex; align-items: center; justify-content: center; }
        .particles { position: fixed; inset: 0; z-index: 1; pointer-events: none; }
        .dot { position: absolute; border-radius: 50%; animation: floatDot linear infinite; opacity: 0; }
        @keyframes floatDot { 0% { opacity: 0; transform: translateY(0) scale(0.5); } 10% { opacity: 0.4; } 90% { opacity: 0.2; } 100% { opacity: 0; transform: translateY(-80vh) scale(1.2); } }
      ...
      <div className="particles" id="bg-particles"></div>
      <div className="logo-track row1" id="bg-row1"></div>
      <div className="logo-track row2" id="bg-row2"></div>
      <div className="logo-track row3" id="bg-row3"></div>
      <div className="logo-track row4" id="bg-row4"></div>
...
function buildTracks() {
  ...
  rows.forEach((row, i) => {
    const track = document.getElementById(`bg-row${i+1}`)
    if (!track) return
    for (let r = 0; r < 3; r++) {
      row.forEach(l => {
