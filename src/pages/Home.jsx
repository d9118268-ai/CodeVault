...
import founderImg from "../assets/Me_in_Suit.png"
    ...
    <>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { background: #000; font-family: 'Segoe UI', sans-serif; overflow-x: hidden; }
        ...
        .wave-canvas { position: fixed; inset: 0; z-index: 1; opacity: 0.18; pointer-events: none; }
        ...
        .hero { position: relative; z-index: 10; min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 0 20px; }
        .brand-tag { font-size: 11px; letter-spacing: 4px; color: #8855ff; text-transform: uppercase; }
        .hero-title { font-size: clamp(2.8rem, 6vw, 5.5rem); font-weight: 900; line-height: 1.05; background: linear-gradient(135deg, #fff 30%, #a066ff 70%, #4488ff 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; letter-spacing: -1px; margin: 16px 0 12px; }
        .hero-sub { font-size: 1.1rem; color: rgba(255,255,255,0.45); max-width: 420px; line-height: 1.6; }
        .btn-row { display: flex; gap: 16px; align-items: center; margin-top: 28px; flex-wrap: wrap; justify-content: center; }
        .btn-signup { padding: 14px 36px; font-size: 1rem; font-weight: 700; border: none; border-radius: 40px; background: linear-gradient(135deg, #4466ff 0%, #8833dd 100%); color: #fff; cursor: pointer; box-shadow: 0 0 28px rgba(100,80,255,0.55); transition: transform 0.2s; }
        .btn-signup:hover { transform: translateY(-2px); }
        .btn-signin { padding: 14px 36px; font-size: 1rem; font-weight: 600; border-radius: 40px; background: transparent; color: #fff; cursor: pointer; border: 1.5px solid rgba(255,255,255,0.2); transition: background 0.2s, transform 0.2s; }
        .btn-signin:hover { background: rgba(255,255,255,0.1); transform: translateY(-2px); }
        ...
        .section { position: relative; z-index: 10; padding: 80px 20px; text-align: center; }
        .section-title { font-size: 2rem; font-weight: 800; color: #fff; margin-bottom: 10px; }
        .section-title span { background: linear-gradient(135deg, #a066ff, #4488ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .founder-card { max-width: 600px; margin: 40px auto 0; background: rgba(255,255,255,0.04); border: 1px solid rgba(160,102,255,0.3); border-radius: 20px; padding: 40px; display: flex; flex-direction: column; align-items: center; gap: 24px; }
        .founder-img { width: 160px; height: 160px; border-radius: 50%; object-fit: cover; object-position: top; border: 3px solid #a066ff; box-shadow: 0 0 30px rgba(160,102,255,0.4); }
        .founder-name { font-size: 1.4rem; font-weight: 800; color: #fff; }
        .founder-title { font-size: 0.9rem; color: #a066ff; letter-spacing: 2px; text-transform: uppercase; margin-top: 4px; }
        .founder-bio { font-size: 1rem; color: rgba(255,255,255,0.6); line-height: 1.8; max-width: 480px; }
        .footer { position: relative; z-index: 10; padding: 30px 20px; text-align: center; border-top: 1px solid rgba(255,255,255,0.08); }
        .footer p { color: rgba(255,255,255,0.3); font-size: 0.85rem; }
        .footer span { color: #a066ff; }
      `}</style>
      ...
      <svg className="wave-canvas" viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice">
        ...
        <path fill="none" stroke="url(#wg1)" strokeWidth="1.5" opacity="0.7">
          <animate attributeName="d" dur="9s" repeatCount="indefinite" values="M0 450 C200 350,400 550,600 450 S900 300,1100 450 S1350 550,1440 450;M0 420 C250 520,450 320,650 420 S950 570,1150 420 S1380 300,1440 420;M0 450 C200 350,400 550,600 450 S900 300,1100 450 S1350 550,1440 450"/>
        </path>
        <path fill="none" stroke="url(#wg2)" strokeWidth="1" opacity="0.5">
          <animate attributeName="d" dur="12s" repeatCount="indefinite" values="M0 500 C300 400,500 600,720 500 S1050 350,1250 500 S1420 600,1440 500;M0 480 C280 580,480 380,700 480 S1030 630,1230 480 S1410 380,1440 480;M0 500 C300 400,500 600,720 500 S1050 350,1250 500 S1420 600,1440 500"/>
        </path>
      </svg>
      ...
      <div className="section">
        <p className="section-title">Meet the <span>Founder</span></p>
        <div className="founder-card">
          <img src={founderImg} className="founder-img" alt="Founder" />
          <div>
            <div className="founder-name">Idowu David Ayomide 🇳🇬</div>
            <div className="founder-title">Founder & CEO — Elon Jr. Tech LTD</div>
            <p className="founder-bio" style={{marginTop:"16px"}}>
              My name is Idowu David Ayomide, a 17-year-old self-taught developer from Nigeria 🇳🇬, popularly known as Elon Jr. I built CodeVault from scratch with one mission — to make world-class coding education accessible to everyone, everywhere. Growing up in Nigeria, I saw how many talented young people lacked access to quality tech education. CodeVault is my answer to that problem. Under my company Elon Jr. Tech LTD, I'm building a suite of platforms that will change how Africa learns. I believe the next generation of world-changing developers is in Africa — and I'm here to build them.
            </p>
          </div>
        </div>
      </div>
      ...
      <footer className="footer">
        <p>© 2026 <span>CodeVault</span> — A product of <span>Elon Jr. Tech LTD</span> 🇳🇬</p>
        <p style={{marginTop:"8px"}}>Built with 💜 by Idowu David Ayomide</p>
      </footer>
    </>
  )
}
...
function buildTracks() {
  const langs = [
    ...
    { name:'JavaScript', color:'#F7DF1E', html:`<svg viewBox="0 0 128 128"><rect width="128" height="128" fill="#F7DF1E"/><path d="M116 18H12v92h104V18zm-31.6 74.4c0 9.7-5.7 14.1-14 14.1-7.5 0-11.8-3.9-14-8.6l7.6-4.6c1.5 2.6 2.8 4.8 6 4.8 3.1 0 5-1.2 5-5.9V55.1h9.4v37.3zm-30.9.6c0 11.3-6.6 16.4-16.2 16.4-8.7 0-13.7-4.5-16.3-9.9l7.6-4.7c1.9 3.3 4.4 5.8 8.8 5.8 3.7 0 6.1-1.8 6.1-6.1V55.1h10v37.9z" fill="#323330"/></svg>` },
    { name:'HTML5', color:'#E34F26', html:`<svg viewBox="0 0 128 128"><path d="M9.5 115.9L24.3 128l14.8-12.1V6.6H9.5v109.3z" fill="#E44D26"/><path d="M64 103.5l25.3-7 3.5-39.2H64V40.8h32.5l1-11H64V19H98l3.5-13H64v98z" fill="#F16529"/><path d="M64 57.3v11.5h20.8l-2 22-18.8 5.1V96l25.3-7.1 3.5-38.6H64z" fill="#EBEBEB"/></svg>` },
  ...
  const rows = [langs.slice(0,4), langs.slice(4,8), langs.slice(8,12), [langs[0],langs[2],langs[5],langs[9],langs[1],langs[7]]]
  rows.forEach((row, i) => {
